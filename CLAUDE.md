...
| **A merchant key can carry a scheme prefix, and it still has exactly one `@`** | `upi:WFMLMH2@ybl` passes every obvious VPA check and is not an address — composing from it yields `pa=upi%3AWFMLMH2%40ybl`, a payment app opening on a payee no PSP can resolve. **A VPA never contains a colon.** Found by two existing tests failing on *button emphasis* — the "grep for the assertion" rule arriving from the other side: a test named for one thing catching a worse one. `F-195` |
