...
| **The MCC is never returned to the paying app, so a ₹1 probe buys nothing** | The category is attached by the **acquirer** during settlement and travels acquirer→issuer, *around* the payer's phone. A UPI response to a TPAP carries status, an RRN and a reference. So "pay ₹1 and read the category off the confirmation" spends the rupee and returns nothing — and a UPI push to a merchant has no reversal available to the payer. `docs/49` §3 |
| **SWIP cannot be an Account Aggregator FIU, and that is a consequence of what makes it shippable** | Sahamati: *"you cannot be an FIU or FIP if you are not a regulated entity"* — RBI, SEBI, IRDAI, PFRDA or Dept of Revenue. SWIP does not touch money, which is its defining property and exactly what disqualifies it. "Partner with a licensed FIU" is not a workaround: the FIU is who the consent is granted **to**. `docs/49` §4 |
| **The ReBIT AA deposit schema has no MCC field** | `Transaction` carries `type, mode, amount, currentBalance, transactionTimestamp, valueDate, txnId, narration, reference` — that is all. A category on a bank statement line lives in the **free-text narration**, in that bank's own format, obliged by nothing. "Federal Bank shows it" is true and does not generalise. `docs/49` §4 |
| **A prepaid-wallet route to the MCC is a licence problem, not an engineering one** | It works — an authorisation message reaching the issuer's switch carries the MCC whether approved or declined. But you must **be** the issuer: a Certificate of Authorisation under the PSS Act 2007 and **₹5 crore net worth at application, ₹15 crore within three years**. A BIN sponsor drops the licence, not the capital or the compliance. That is a different company, not a phase two. `docs/49` §5 |
| **`QUERY_ALL_PACKAGES` is a restricted permission; a `<queries>` intent signature is not** | Listing the phone's UPI apps needs package visibility, and the broad permission is granted by Play only when inventory visibility **is** the app's core purpose. SWIP's is a shop's category. The narrow form — "who can VIEW a `upi:` URI" — needs no permission and is structurally incapable of being an inventory. Same trade `docs/36` D-45 declined for Doze. `F-194` |
| **A signed QR must be forwarded byte for byte, never rebuilt from its fields** | `sign=` is a DER ECDSA signature **over the rest of the payload**, and `mode`, `orgid` and `tr` ride with it. Recomposing `upi://pay?pa=…&pn=…` from two parsed fields drops all of it: the merchant loses verified status, or the code is rejected outright. Forward the original string; compose only when there is none. `F-194` |
| **A merchant key can carry a scheme prefix, and it still has exactly one `@`** | `upi:WFMLMH2@ybl` passes every obvious VPA check and is not an address — composing from it yields `pa=upi%3AWFMLMH2%40ybl`, a payment app opening on a payee no PSP can resolve. Found by two existing tests failing on *button emphasis* — the "grep for the assertion" rule arriving from the other side: a test named for one thing catching a worse one. `F-195` |
| **And the fix that was safe was also wrong, which is the sharper half** | Rejecting every colon made the suite green in one line. But `CaptureResolver` writes **`upi:<vpa>` as the canonical key for every UPI capture** (`capture_resolver.dart:76`) — so "reject" silently disabled the compose-from-address fallback for *the only kind of row it exists for*, and nothing would ever have failed. **Strip the one prefix you wrote yourself; refuse the ones you did not.** A green suite is not evidence that a guard guards the right thing. `F-196` |
| **A UPI intent and a UPI QR are the same string in a different envelope** | NPCI's linking spec gives one URL format and lists *"QR, intent, NFC, BLE, UHF"* as carriers of it. `mc` is **optional in both** and its own wording is *"if present then needs to be passed as it is"* — **forwarded, never generated**. So converting a scanned QR into an intent cannot produce a category. Intents in the wild do carry `mc` more often, and the cause is that online aggregators onboard merchants properly: **the acquirer decides, not the envelope.** `docs/51` |
| **The category for a sticker-only shop is often on the shop's *other* sticker** | `docs/42` measured it: every Google Pay for Business code publishes `mc`, not one Paytm/PhonePe/BharatPe code does. A counter with both has the answer taped to it a foot from the code that was scanned. `MerchantReconciler` refused `upi:`↔`upi:` links on the grounds that *two QRs in one cell are two shops in a market* — true for two codes from the **same** PSP, false for two from different ones. Opened up behind *different acquirers* + a three-minute window + the existing confirmation. `F-198` |
| **Widening what a feature fires on can make old copy wrong without touching it** | `MerchantLinkCard` said *"and scanned X two minutes **later**"* while `minutesApart` has always been an `.abs()` — it never knew which came first. Survivable while the pair was a tap and a scan, where the tap usually is first. **Two stickers have no natural order**, so opening that case up would have made the sentence backwards about half the time, in a prompt whose whole job is helping somebody decide if two things are the same shop. *Apart*, not *later*. **After widening a trigger, re-read every sentence written for the old one.** `F-198` |
| **A null is a refusal, not a maybe, in any rule shaped "they are different"** | The two-sticker link turns on the acquirers differing. An unknown acquirer on either side would satisfy `a != b` while meaning *I do not know who issued this* — the one state in which difference must not be assumed. Refuse on null before comparing. `F-198` |
| **Inside a `try`, `return f()` and `return await f()` are different programs** | A bare `return` hands the caller an unresolved future and **leaves the try block first**, so a failure inside `f` lands on whoever awaits it rather than in the `catch` two lines down — the screen shows an exception where it was meant to show an empty state. `analyze` names it `unawaited_return_in_try_block`, and it is a **warning** where most of this project's analyzer output is `info`: infos have never failed the build, one warning does. `F-197` |
| **`find.byType` matches `runtimeType ==`, so an abstract base class finds nothing** | `find.widgetWithText(ButtonStyleButton, 'View all')` resolves through `byType` and comes back **empty** — and the failure reads as *the button is missing*, which is a different and much more alarming bug than the one you have. `find.bySubtype<T>()` is the one that walks the hierarchy. Reaching for a base class to stop pinning a concrete widget is right; that finder is not the way to do it. `F-196` |
| **A third action re-prices the two beside it** | Adding *Continue payment* did not merely add a button: it took the filled slot, so two tests asserting `FilledButton, 'View all'` broke. Both were **right before and right after** — the behaviour genuinely changed. The repair is not to re-pin the shape but to assert the thing the test is about: `ButtonStyleButton` for "is it disabled", the shape only where emphasis *is* the subject. **Pin the property under test, not the widget that happens to carry it.** `F-196` |
...
| **Every route to an MCC, priced, with what each one needs** | [`docs/49-EVERY-ROUTE-TO-AN-MCC.md`](docs/49-EVERY-ROUTE-TO-AN-MCC.md) |
| **Every ask from prompt 54, quoted, with status** | [`docs/50-PROMPT-54-REGISTER.md`](docs/50-PROMPT-54-REGISTER.md) |
| **Whether a pay-by-app intent can carry an MCC a QR cannot** | [`docs/51-CAN-AN-INTENT-CARRY-AN-MCC-A-QR-DOES-NOT.md`](docs/51-CAN-AN-INTENT-CARRY-AN-MCC-A-QR-DOES-NOT.md) |
