...
| **SWIP cannot use Android's own notification Bubbles API** | From API 30 a bubble's notification must reference a **sharing shortcut**, and bubbles are scoped to conversations and a `Person`. SWIP has a shop code, not a person. So `SYSTEM_ALERT_WINDOW` is not a shortcut around a nicer API — it is the only route open to this app. `docs/39` §4 |
