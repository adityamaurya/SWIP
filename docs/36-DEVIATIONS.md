...
| ⏸ D-58 | Keeping `BubbleTrace` after its stated deletion condition was met | **`F-178` is confirmed by a real export ([`38` §6](38-BUBBLE-TRACE.md)), so by `38` §4 the recorder should have gone this round.** Held for one more, because the same export found `F-180`, and that fix can only be proved by days of ordinary use — snoozing, backgrounding, rebooting, the screen going off — none of which runs on a CI machine with no screen. Removing the only instrument that can read the result in the same round as the fix means the next report is a description again. Recorded as a deviation rather than done quietly, with the exit condition written down: a second export showing `bubble.restored` at an edge. [`38` §8](38-BUBBLE-TRACE.md) |
| ⏸ D-54 | Temporary debugging apparatus in the shipped tree | **Accepted, with the removal made mechanical rather than remembered.** `BubbleTrace` is gated on `FLAG_DEBUGGABLE`, so it cannot reach a Play Store build at all, and `check_wiring.py` fails if that gate is weakened. [`38`](38-BUBBLE-TRACE.md) has the ten-step deletion list, including deleting the guard. `F-176` |
