...
| 🟣 D-55 | `F-170` cited `springy-heads` and `google/hover` as live prior art | **Both are dead** — `springy-heads` unmaintained and pointing at Hover, Hover archived by Google in Jan 2023. The decision they were cited to support (`androidx.dynamicanimation` over adopting a library) was **right for a better reason than the one given**: either would have been inherited abandoned code. Citations corrected. [`39` §1](39-FLOATING-OVERLAY-PRIOR-ART.md) |
| 🚫 D-56 | Using Android's own notification Bubbles API instead of `SYSTEM_ALERT_WINDOW` | **Not possible, checked rather than assumed.** From API 30 a bubble must reference a sharing shortcut and is scoped to a conversation and a `Person`. SWIP has a shop code. Faking a conversation would misrepresent the app to the system and to Play review. The overlay permission is the only route open. [`39` §4](39-FLOATING-OVERLAY-PRIOR-ART.md) |
| 🟣 D-52 | `F-170`: the hover card should *rise* from below, because the bubble that summoned it is down there | **Wrong, and the owner felt it as "another app opening".** Entering from the bottom edge is Android's grammar for a new activity, however small the distance — 6% was enough to read as one. A scale has no direction and resolves in place. `F-177` |
| 🟣 D-53 | `F-172`: a shadow under the Close button is honest, because it really is floating | **Reversed.** True, and beside the point: a shadow is one more thing saying *a window has been placed on top of yours*, which is the feeling being removed. Safe to drop because `hover_chrome_test.dart` shows the contrast was never carried by it. `F-177` |
| 🟣 D-57 | `F-175`: *View all* should unfold in place, because a second screen is a second thing to dismiss | **Reversed on instruction, and the instruction was right for a reason the original missed.** Unfolding in place answered a question nobody had asked yet — the full breakdown of a capture taken two seconds ago is not urgent, and the capture has just been *saved*, so the useful destination is where all of them live. It also deleted a state: a collapsed form, an expanded form, two heights and a label that flipped, all in front of a live camera. `F-180` |
| ⏸ D-58 | Keeping `BubbleTrace` after its stated deletion condition was met | **`F-178` is confirmed by a real export ([`38` §6](38-BUBBLE-TRACE.md)), so by `38` §4 the recorder should have gone.** Held again, and the reason got stronger rather than weaker: the export meant to confirm `F-180` instead found `F-184` **inside** `F-180`'s own fix — the Y half of the resting position, still being written on every `ACTION_MOVE`. Twice now the instrument has found the thing after the thing, and each of those faults is invisible while the window is hidden. Recorded as a deviation rather than done quietly, with the exit condition written down: an export showing `bubble.restored` at the edge the bubble was parked at. [`38` §8](38-BUBBLE-TRACE.md) |
| ⏸ D-54 | Debugging apparatus in the shipped tree | **Accepted, with the removal made mechanical rather than remembered.** All three recorders are gated on `FLAG_DEBUGGABLE`, so none can reach a Play Store build, and `check_wiring.py` fails if either gate is weakened. `F-187` deliberately wrote the second gate check as its **own function** rather than extending the first, because [`38` §4](38-BUBBLE-TRACE.md) step 9 deletes the first — a shared one would have taken the surviving guarantee with it. Only `BubbleTrace` is temporary; the POS and battery boxes are the owner's standing ritual. `F-176`, `F-187` |
...
| 🟣 D-59 | `F-163`: cover the half second before the hovering engine starts with a `Scanning…` label on the bubble | **Wrong, and the owner caught it in one sentence** — *"It opens as something called 'scanning.' It's actually not scanning, right?"* It was not. The window had not started, its Flutter engine had not started and the camera was half a second away. Covering a gap is reasonable; **naming it something it is not is not.** And the label was also the whole reason the bubble could stop being a circle: showing a word widened a `WRAP_CONTENT` row over a rectangle whose corner radius is a circle only while square. One bad idea, reported as two separate complaints. `F-185` |
| 🟣 D-60 | `F-187`: hold the three channel names as fields on the `Blackbox` enum, which is tidier | **Wrong, and my own gate said so within the minute.** `check_wiring` reported six handlers as dead platform code, because its rule is *literals at the call site* — and that constraint **is the feature**: a name it cannot see is a name it cannot match against `MainActivity.kt`. Tidier code that switches off a check is a net loss. The literals went back into a `switch`, with the reason written above it. `F-187` |
| 🟣 D-61 | `F-180`: the capture result should never show the field table — *"the ledger does that now"* | **Reversed for one of the two windows, on instruction.** The argument was sound and it was about the hovering card: at a counter, over somebody else's app, the breakdown is detail nobody stopped to read covering something they did. Applying it to the **full-screen** scanner was the overreach — that is SWIP's own screen with a viewfinder behind it, opened deliberately, where the detail costs nothing. One rule, two windows, and only one of them was ever the reason for it. `F-189` |
| 🟣 D-62 | `F-189`, my first version: put the two fan items at +30° and −30°, symmetric about the horizontal | **Still a list, and I nearly shipped it.** Symmetric angles **share a cosine**, so both items land on the same `x` — a column in front of the bubble instead of a column above it. It answers the letter of *"in front"* and none of *"circular… like tree branches"*. Caught by printing the numbers rather than by reading the code, which is the only reason it was caught at all. A 25° tilt puts them at 55° and −5°. `F-189` |
| 🟣 D-63 | Every round's reply linked its evidence as `docs/NN-FILE.md`, a repo-relative path | **Dead in a chat window, in every reply, since round one.** The standing rule is *never leave the owner blind with words* and the links were satisfying it on paper while opening nothing — *"Unsupported link"*. A link that looks like a link and does nothing is worse than plain text, because nobody reports plain text. Found only because the owner screenshotted the error. Absolute `blob/<branch>/` URLs now, and the rule is in `CLAUDE.md`'s standing table where it cannot be forgotten. `F-190` |
