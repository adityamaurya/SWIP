...
| Floating bubble | 32, 36 | ◑ steps 1–3 built (`F-131`, `F-158`) — the service, the overlay, drag, edge-snap and all three suppression rules. **Step 4, the in-overlay camera, is open**: CameraX is a Gradle dependency and `build.gradle` is regenerated by `bootstrap.sh`, [`32` §9](32-FLOATING-BUBBLE.md). Today the bubble opens SWIP's own scanner |
...
| Bubble step 4 — the in-overlay camera | Needs `bootstrap.sh` to inject Gradle dependencies into a file it generates. Its own change with its own failure modes | Mine, next round |
